...
type withdrawalDaemon struct {
	// polygon proxy server connection
...
func (w *withdrawalDaemon) initialize() {
	...
	_instance, err = w.instanceRepo.FindServerByName(w.instanceRepo.Conn(), constant.PolygonWithdrawalDaemon)
	if err != nil {
		panic(err)
	}
...
func (w *withdrawalDaemon) InsertErrLog(err error) error {
	...
	_, _err = w.instanceRepo.InsertErrorLog(w.instanceRepo.Conn(), &entity.FinexblockErrorLog{
		...
		Process:     constant.PolygonWithdrawalDaemon,
		Priority:    entity.HIGH,
		Description: "Error on Polygon withdrawal daemon",
		Err:         err.Error(),
...
func (w *withdrawalDaemon) ScanWithdrawalRequests(tx *gorm.DB, status entity.WithdrawalStatus) ([]*entity.WithdrawalRequest, error) {
	return w.walletRepo.ScanWithdrawalRequestByCond(tx, constant.PolygonCoinID, status)
}
...
func (w *withdrawalDaemon) Transfer(ctx context.Context, from, to string, amount decimal.Decimal) (*polygon.SendRawTransactionOutput, error) {
	return w.polygon.SendRawTransaction(ctx, &polygon.SendRawTransactionInput{
		From:   from,
		To:     to,
...
func (w *withdrawalDaemon) GetReceipt(ctx context.Context, txHash string) (*polygon.GetReceiptOutput, error) {
	return w.polygon.GetReceipt(ctx, &polygon.GetReceiptInput{TxHash: txHash})
}
...
func (w *withdrawalDaemon) ProcessPendingWithdrawalRequest() error {
	return w.walletRepo.Conn().Transaction(func(tx *gorm.DB) error {
		...
		var output *polygon.GetReceiptOutput
...
func (w *withdrawalDaemon) ProcessApprovedWithdrawalRequest() error {
	return w.walletRepo.Conn().Transaction(func(tx *gorm.DB) error {
		var requests []*entity.WithdrawalRequest
		var output *polygon.SendRawTransactionOutput
...
func newWithdrawalDaemon(polygon polygon.PolygonProxyClient, centralWalletAddress string, db *gorm.DB, client *redis.ClusterClient, interval time.Duration) *withdrawalDaemon {
	return &withdrawalDaemon{
		...
		logger:               files.NewWriter("polygon-withdrawal-daemon", "polygon-withdrawal-daemon"),
