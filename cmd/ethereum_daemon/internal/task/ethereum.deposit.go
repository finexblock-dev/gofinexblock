...
func newDepositDaemon(
	proxy ethereum.EthereumProxyClient,
	centralWalletAddress string,
	db *gorm.DB,
	...
	return &depositDaemon{
		...
		logger:               files.NewWriter("ethereum-deposit-daemon", "ethereum-deposit-daemon"),
	}
}
...
func (d *depositDaemon) initialize() {
	...
	_instance, err = d.instanceRepo.FindServerByName(d.instanceRepo.Conn(), constant.EthereumDepositDaemon)
	if err != nil {
		panic(err)
	}
...
func (d *depositDaemon) InsertErrLog(err error) error {
	...
	_, _err = d.instanceRepo.InsertErrorLog(d.instanceRepo.Conn(), &entity.FinexblockErrorLog{
		...
		Process:     "Ethereum Deposit Daemon",
		Priority:    entity.HIGH,
		Description: "Error on Ethereum Deposit Daemon",
		Err:         err.Error(),
...
func (d *depositDaemon) GetCurrentBlockNumber(ctx context.Context) (uint64, error) {
	var output *ethereum.GetBlockNumberOutput
	var err error

	output, err = d.ethereum.GetBlockNumber(ctx, &ethereum.GetBlockNumberInput{})
	if err != nil {
		return 0, err
	}
...
func (d *depositDaemon) GetBlock(ctx context.Context, start, end decimal.Decimal) ([]*ethereum.TxData, error) {
	var output *ethereum.GetBlocksOutput
	var err error

	output, err = d.ethereum.GetBlocks(ctx, &ethereum.GetBlocksInput{Start: uint64(start.IntPart()), End: uint64(end.IntPart())})
	if err != nil {
...
func (d *depositDaemon) GetBalance(ctx context.Context, address string) (*ethereum.GetBalanceOutput, error) {
	return d.ethereum.GetBalance(ctx, &ethereum.GetBalanceInput{Address: address})
}
...
func (d *depositDaemon) ScanWallets(tx *gorm.DB) ([]*entity.Wallet, error) {
	return d.walletRepo.ScanWalletByCoinID(tx, constant.EthereumCoinID)
}
...
func (d *depositDaemon) Transfer(ctx context.Context, from, to string, amount decimal.Decimal) (*ethereum.SendRawTransactionOutput, error) {
	return d.ethereum.SendRawTransaction(ctx, &ethereum.SendRawTransactionInput{From: from, To: to, Amount: amount.String()})
}

func (d *depositDaemon) CheckReceipt(txHash string) (*ethereum.GetReceiptOutput, error) {
	return d.ethereum.GetReceipt(context.Background(), &ethereum.GetReceiptInput{TxHash: txHash})
}
...
func (d *depositDaemon) Gathering(ctx context.Context) {
	...
	if err = d.walletRepo.Conn().Transaction(func(tx *gorm.DB) error {
		...
		var _transfer *ethereum.SendRawTransactionOutput
		var _balance *ethereum.GetBalanceOutput
		...
		for _, _wallet = range wallets {
			...
			_transferValue = _value.Sub(constant.EthereumMinimumGatheringAmount)
...
func (d *depositDaemon) Task() error {
	...
	return d.walletRepo.Conn().Transaction(func(tx *gorm.DB) error {
		...
		blockNumber, err = d.walletRepo.FindBlockNumberByCoinID(tx, constant.EthereumCoinID)
		if err != nil {
		...
		if _, err = d.walletRepo.UpdateBlockNumber(tx, constant.EthereumCoinID, to.Add(decimal.NewFromFloat(1)), to.Add(decimal.NewFromFloat(10))); err != nil {
		...
		return d.walletRepo.Conn().Transaction(func(tx2 *gorm.DB) error {
			...
			if err != nil {
				d.Log("insert error log", d.InsertErrLog(fmt.Errorf("error while get blocks: from block number: %v, to block number: %v, error: [%v]", from.IntPart(), to.IntPart(), err)))
				// when case of error, just go on.
				return nil
			}
			...
			for _, txData := range blocks {
				if err = tx2.Transaction(func(tx3 *gorm.DB) error {
					var receipt *ethereum.GetReceiptOutput
					...
					// Acquire lock
					if ok, err := d.AcquireLock(_user.UUID, constant.ETH); !ok || err != nil {
					...
					// Plus balance
					if err = d.tradeManager.PlusBalance(_user.UUID, constant.ETH, amount); err != nil {
