...
type EthereumKeyConfig struct {
	sess *session.Session
}

type EthereumKeyAssumeRoleStruct struct {
...
type EthereumKeyConfiguration struct {
...
func NewEthereumKeyConfig() (*EthereumKeyConfig, error) {
	sess, err := secure.GetSessionFromEnv()
	if err != nil {
		return nil, err
	}
	return &EthereumKeyConfig{sess: sess}, nil
}

func (e *EthereumKeyConfig) SetSess(sess *session.Session) {
	e.sess = sess
}

func (e *EthereumKeyConfig) AssumeRole(roleArn string, sessionName string) (*sts.AssumeRoleOutput, error) {
	stsClient := secure.NewSTSClient(e.sess)
	return secure.AssumeRole(stsClient, roleArn, sessionName, 900)
}

func (e *EthereumKeyConfig) GetSecretValue(secretName string) (*secretsmanager.GetSecretValueOutput, error) {
	secretClient := secure.NewSecretMangerClient(e.sess)
	return secure.GetSecretValue(secretClient, secretName)
}

func (e *EthereumKeyConfig) Decrypt(cipherText []byte, keyID string) (*kms.DecryptOutput, error) {
	kmsClient := secure.NewKMSClient(e.sess)
	return secure.Decrypt(kmsClient, cipherText, keyID)
}

func (e *EthereumKeyConfig) Credentials(secretName string, target interface{}) {
	var assumeRoleStruct *EthereumKeyAssumeRoleStruct
